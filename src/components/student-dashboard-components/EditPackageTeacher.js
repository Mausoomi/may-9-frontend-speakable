...
const EditPackageTeacher = () => {
  ...
  const handleDeleteSlot = (date, innerIndex, slotIndex, outkey) => {
    // Create a copy of AllSlots
    const updatedAllSlots = [...AllSlots];

    // Find the object corresponding to the provided outkey
    const slotObj = updatedAllSlots.find((obj) => obj[outkey]);

    if (slotObj) {
      // Find the slot array for the provided date
      const slotArray = slotObj[outkey][date];

      if (slotArray) {
        // Remove the slot at the specified innerIndex and slotIndex
        slotArray.splice(innerIndex, 1);

        // Check if the slotArray is empty after deletion
        if (slotArray.length === 0) {
          // If slotArray is empty, remove the entire slotObj for the date
          delete slotObj[outkey][date];

          // Check if slotObj is empty after deletion
          if (Object.keys(slotObj[outkey]).length === 0) {
            // If slotObj is empty, remove the entire slotObj
